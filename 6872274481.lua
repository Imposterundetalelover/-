--[[
       oh hi sir,ur here to skid?
       if u r then fuck off
]]
...
function warnnotify(title, content, duration)
	local frame = GuiLibrary["CreateNotification"](title or "Aqua", content or "(No Content Given)", duration or 5, "assets/WarningNotification.png")
	frame.Frame.Frame.ImageColor3 = Color3.fromRGB(255, 145, 0)
end
function infonotify(title, content, duration)
	local frame = GuiLibrary["CreateNotification"](title or "Aqua", content or "(No Content Given)", duration or 5, "assets/InfoNotification.png")
	frame.Frame.Frame.ImageColor3 = Color3.fromRGB(255, 145, 0)
end
function getstate()
...
run(function()
	...
	function tweenToNearestPlayer()
		...
		if nearestPlayer and not hasTeleported then
			...
			local tween = TweenService:Create(lplr.Character.HumanoidRootPart, TweenInfo.new(0.84), {CFrame = nearestPlayer.Character.HumanoidRootPart.CFrame + Vector3.new(0, 15, 5)})
			tween:Play()
		end
	end

	local Playertp = COB("Blatant", {
		Name = "PlayerTP",
		Function = function(callback)
			if callback then
...
game:GetService("UserInputService").JumpRequest:connect(function()
    ...
    Default = false,
    HoverText = "Infinitely jumping"
})

local Skybox = COB("Skybox", {
    ["Name"] = "VoidSky",
    ["Function"] = function(v)
        if v then
    ...
    ["Function"] = function(v)
        if v then
            game.Lighting.Sky.SkyboxBk = "rbxassetid://1735468027"
				...
				game.Lighting.FogColor = Color3.new(104, 72, 142)
                game.Lighting.FogEnd = "10000"
                game.Lighting.FogStart = "0"
                game.Lighting.Ambient = Color3.new(0.5, 0, 0)
            else
                game.Lighting.Sky.SkyboxBk = "rbxassetid://1735468027"
				game.Lighting.Sky.SkyboxDn = "rbxassetid://1735500192"
				game.Lighting.Sky.SkyboxFt = "rbxassetid://1735467260"
...
run(function()
	...
	Night = GuiLibrary["ObjectsThatCanBeSaved"]["WorldWindow"]["Api"].CreateOptionsButton({
		["Name"] = "Night Time",
		["Function"] = function(callback)
			if callback then
...
local pinkSkull = GuiLibrary["ObjectsThatCanBeSaved"]["SkyboxWindow"]["Api"].CreateOptionsButton({
    ["Name"] = "PinkSky",
    ["Function"] = function(v)
        if v then
...
local Nightv2= GuiLibrary["ObjectsThatCanBeSaved"]["SkyboxWindow"]["Api"].CreateOptionsButton({
    ["Name"] = "NightTimeV2",
    ["Function"] = function(v)
        if v then
...
local Realistic = GuiLibrary["ObjectsThatCanBeSaved"]["SkyboxWindow"]["Api"].CreateOptionsButton({
    ["Name"] = "RealisticSkyV2",
    ["Function"] = function(v)
        if v then
...
local Skyox = COB("Skybox", {
	...
	run(function()
	 ...
	 Ambience12 = GuiLibrary["ObjectsThatCanBeSaved"]["SkyboxWindow"]["Api"].CreateOptionsButton({
		 ["Name"] = "BlueAmbience",
		 ["Function"] = function(callback)
			 if callback then
...
local Messages = {"Aquatint on top!", "LMFAO!", "INF", "Thump!", "KYS", "FUCK", 
}
local old
    ...
    if FunnyIndicator.Enabled then
        ...
        debug.setupvalue(bedwars.DamageIndicator, 10, {
            Create = function(self, obj, ...)
                spawn(function()
                    pcall(function()
                        obj.Parent.Text = particletexttable[math.random(1,#particletexttable)]
						obj.Parent.TextColor3 =  Color3.fromRGB(0,0,255)
                    end)
                end)
